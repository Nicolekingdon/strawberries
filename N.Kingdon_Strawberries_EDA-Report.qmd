...
Finally, I selected the rows that are relevant and renamed `temp3` to `measurement` and `temp4` to `avg`.
...
`Metric` has the following strings:`r paste(strwb_census |> distinct(Metric))`. The empty string holds no values because that row did not need a metric to help describe the data. `$` represents that the metric for the following `Value` column is in US Dollars. `CWT` is weight in 100's \[[CWT](https://mainlinemetals.com/resource-term/hundredweight-cwt/#:~:text=A%20hundredweight%20(CWT)%20measures%20the,in%20large%20volumes%20or%20weight.)\].
...
Finally, the `CV (%)` column holds the co-effcients of variance of the numeric values in the `Value` column.
...
1.  What state held the most operational strawberry sellers?
...
##### What state held the most operational strawberry sellers? 

```{r, echo=FALSE}

filtered_data <- strwb_census |>
  filter(Totals == "OPERATIONS WITH SALES")

ggplot(data = filtered_data,
       mapping = aes(x = State, y = Value)) +
  geom_point() +
  labs(
    title = "States and Organic Operational Strawberry Sales",
    x = "State",
