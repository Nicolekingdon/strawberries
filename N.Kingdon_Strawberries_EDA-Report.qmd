...
strwb_survey_chem$Value <- as.numeric(str_replace_all(strwb_survey_chem$Value,pattern = ",", replacement = "")) 

strwb_survey_chem <- strwb_survey_chem |>
  filter(!is.na(Value))

strwb_survey_chem$Value <- format(strwb_survey_chem$Value, scientific = F)
```
...
[![World Health Organization Pesticide Toxicity Classification](straw_health.png){fig-align="center" width="459"}](https://www.researchgate.net/publication/278283443_Relative_toxicity_of_pesticides_in_the_developing_world)
...
## Review of Data Frames: strwb_census
...
ggplot(data = filtered_data,
  ...
  labs(
    title = "Figure 4. States* and Organic Operational Strawberry Sales",
    x = "Year",
...
ggplot(data = filtered_data4,
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Figure 5. California* and Organic Operational Strawberry Sales",
    x = "Year",
    y = "Operations with Sales",
...
ggplot(data = filtered_data5,
  ...
  geom_text(aes(label = Value), vjust = -0.5, position = position_dodge(width = 0.9)) +
  labs(
    title = "Figure 8. States* and Organic Strawberry Sales",
    subtitle = "in US Dollars",
    x = "Year",
...
ggplot(data = filtered_data6,
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Figure 9. California* and Organic Strawberry Sales",
    subtitle = "in US Dollars",
    x = "Year",
...
## Review of Data Frames: strwb_survey_mkt
...
ggplot(data = filtered_data10,
  ...
  labs(
    title = "Figure 10. Fertilizer Usage on Strawberry Crops",
...
ggplot(data = filtered_data11,
  ...
  facet_wrap(~fertilizer_type, ncol = 2) +
  labs(
    title = "Figure 11. Pounds of Fertilizer Used on Strawberries in California",
    x = "Year",
...
ggplot(data = filtered_data12,
  ...
  labs(
    title = "Figure 12. Pounds of Fertilizer Used on Strawberries in Florida",
    x = "Year",
    y = "lbs of Fertilizer Used (10000s)",
  ) +
  theme_bw() +
...
ggplot(data = filtered_data13,
  ...
  geom_text(aes(label = Value), vjust = -0.5, position = position_dodge(width = 0.9)) +
  labs(
    title = "Figure 13. Pounds of Fertilizer Used Per Acre Per Application",
...
## Review of Data Frames: strwb_survey_treat
...
`measurement` has the following strings that represents the metric that the values were measured in: `r paste(strwb_survey_treat |> distinct(measurement))`.
...
`toxicity` holds the following strings: `r paste(strwb_survey_treat |> distinct(toxicity))`. These are based on the World Organization Health Organization's Pesticide Toxicity Classification \[[WHO, 2019](https://www.who.int/publications/i/item/9789240005662)\] (see more information above).
...
2.  Were bacterium or chemical pesticides more toxic?

##### What pesticide category was used the most?

In Figure 12 (Oregon and Washington), Figure 13 (California), Figure 14 (Florida), there are bar charts to illustrate what category of pesticide was used in each state. The years were 2016 for all the states, with 2018, 2019, and 2021 for Oregon, Washington, and California. The pesticide used most for Oregon and Florida were fungicides, herbicide for Washington, and other for California.
...
ggplot(data = new_data,
  ...
  geom_bar(stat = "identity", position = "dodge") +  
  labs(
    title = "Figure 14. Pounds of Different Pesticide Categories Used",
...
ggplot(data = new_data2,
  ...
  geom_bar(stat = "identity", position = "dodge") +  
  labs(
    title = "Figure 15. Pounds of Different Pesticide Categories Used",
...
ggplot(data = new_data3,
  ...
  geom_bar(stat = "identity", position = "dodge") +  
  labs(
    title = "Figure 16. Pounds of Different Pesticide Categories Used",
    subtitle = "in Florida",
    x = "Year",
    y = "lbs (10000s)"
  ) +
...
##### Were bacterium or chemical pesticides more toxic?

In Table 1 below, it shows the toxicity classified by WHO, as well as the pesticide split up by type. After organizing the table, it is clear that the chemical-related pesticides are often more toxic than bacterium. Only 2 out of 13 bacterium were categorized as "unlikely to present acute hazard", one as "unknown", and the rest being "no significant acute toxicity" (n = 10). For chemicals, there were significantly less pesticides with "no significant acute toxicity" (n = 8) than the bacteria. There was one "highly hazardous" chemical, 21 "moderately hazardous" chemicals, 12 "slightly hazardous", 22 "unlikely to present acute hazard", two "fatal if inhaled", and three "toxic if inhaled".

+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Toxicity                         | Chemicals                         | Bacterium                                           |
+==================================+===================================+=====================================================+
| Highly hazardous                 | "ABAMECTIN"                       | N/A                                                 |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Moderately hazardous             | "MYCLOBUTANIL"                    | N/A                                                 |
|                                  |                                   |                                                     |
|                                  | "PROPICONAZOLE"                   |                                                     |
|                                  |                                   |                                                     |
|                                  | "TETRACONAZOLE"                   |                                                     |
...
|                                  | "METAM-SODIUM"                    |                                                     |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Slightly hazardous               | "BORAX DECAHYDRATE"               | N/A                                                 |
|                                  |                                   |                                                     |
|                                  | "FLUOPYRAM"                       |                                                     |
|                                  |                                   |                                                     |
|                                  | "FLUXAPYROXAD"                    |                                                     |
...
|                                  | "SULFENTRAZONE"                   |                                                     |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Unlikely to present acute hazard | "AZOXYSTROBIN"                    | "NEEM OIL"                                          |
|                                  |                                   |                                                     |
|                                  | "BOSCALID"                        | "BACILLUS PUMILUS"                                  |
|                                  |                                   |                                                     |
|                                  | "CAPTAN"                          |                                                     |
|                                  |                                   |                                                     |
|                                  | "CYFLUFENAMID"                    |                                                     |
...
|                                  | "ISOFETAMID"                      |                                                     |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| No significant acute toxicity    | "MEFENOXAM"                       | "REYNOUTRIA SACHALINE"                              |
|                                  |                                   |                                                     |
|                                  | "POLYOXIN D ZINC SALT"            | "BACILLUS AMYLOLIQUEFACIENS STRAIN D747"            |
|                                  |                                   |                                                     |
|                                  | "POTASSIUM BICARBON."             | "BACILLUS SUBTILIS"                                 |
|                                  |                                   |                                                     |
|                                  | "AZADIRACHTIN"                    | "BT KURSTAK ABTS-1857"                              |
|                                  |                                   |                                                     |
|                                  | "METHOXYFENOZIDE"                 | "BT KURSTAKI SA-11"                                 |
|                                  |                                   |                                                     |
|                                  | "METAM-POTASSIUM"                 | "CHROMOBAC SUBTSUGAE PRAA4-1 CELLS AND SPENT MEDIA" |
|                                  |                                   |                                                     |
|                                  | "CARFENTRAZONE-ETHYL"             | "BLAD"                                              |
|                                  |                                   |                                                     |
|                                  | "IRON PHOSPHATE"                  | "BT SUBSP KURSTAKI EVB-113-19"                      |
|                                  |                                   |                                                     |
|                                  |                                   | "BT SUB AIZAWAI GC-91"                              |
|                                  |                                   |                                                     |
|                                  |                                   | "BURKHOLDERIA A396 CELLS & MEDIA"                   |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Fatal if inhaled                 | "PYRACLOSTROBIN"                  | N/A                                                 |
|                                  |                                   |                                                     |
|                                  | "CHLOROPICRIN"                    |                                                     |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Toxic if inhaled                 | "QUINOLINE"                       | N/A                                                 |
|                                  |                                   |                                                     |
|                                  | "DICHLOROPROPENE"                 |                                                     |
|                                  |                                   |                                                     |
|                                  | "PEROXYACETIC ACID"               |                                                     |
+----------------------------------+-----------------------------------+-----------------------------------------------------+
| Unknown                          | N/A                               | "BT KURSTAKI ABTS-351"                              |
+----------------------------------+-----------------------------------+-----------------------------------------------------+

: Pesticides and Toxicity Level by Chemical and Bacteria \[[WHO, 2019](https://www.who.int/publications/i/item/9789240005662)\]

{{< pagebreak >}}

# Conclusion

In conclusion, the [USDA-NASS](https://quickstats.nass.usda.gov/results/45FBC825-B104-38E2-9802-839F5F3C7036) data set on US states' strawberry production shows US Census data on organic strawberry production (`strwb_census`), and the survey data is on fertilizer usage (`strwb_survey_mkt`) and pesticide usage (`strwb_survey_treat`). This data is prepared for more extensive analysis, specifically in `strwb_survey_treat` data frame. Overall, this data supports in a better understanding of organic versus non-organic strawberry production, as well as production in general for US states.
